...
class MediaStreamDataAdapter(list: ArrayList<MediaStream>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.mediastream_data, parent, false)
        ...
        viewHolder.itemView.setOnClickListener {
            Toast.makeText(
                parent.context, "you click what?", Toast.LENGTH_SHORT
            ).show()
        }
        /* 播放菜单点击页面 */
        viewHolder.streamDescription.setOnClickListener {
            ...
            MainActivity.mainActivityTodo(
                STARTDETAILACTIVITY, mediaStreamDataList[position].streamPath
            )  // 跳转去MainActivity执行相关操作
        }
        /**
         *  长按监听：删除item
         */
        viewHolder.itemView.setOnLongClickListener {
            ...
            MainActivity.mainActivityTodo(
                MainActivity.HANDLELONGCLIECK, mediaStreamDataList[position].streamName
            )
            /* 在ArrayList中移除此码流 */
